      ...
      <div className="w-full text-left py-4 border-b border-stone-200/40 animate-pulse">
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="w-full lg:w-[320px] shrink-0 space-y-4">
            <div className="w-full rounded-[6px] relative overflow-hidden flex flex-col justify-end p-6 min-h-[200px] lg:min-h-[230px] bg-stone-200 dark:bg-stone-855 shadow-sm" />
...
const Clearance_SaleGridInner: React.FC<Clearance_SaleGridProps> = ({
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const start = sale?.start_date ? new Date(sale.start_date).getTime() : 0;
      const end = sale?.end_date ? new Date(sale.end_date).getTime() : 0;
  ...
  const resolvedBannerImg = sale?.image
    ? resolveImageUrl(sale.image)
  ...
  const handleBannerClick = (e: React.MouseEvent) => {
    if (onNavigate) {
      ...
      onNavigate(FASHION_ROUTES.getOffers(ownerUserId, storeSlug, 'clearance'));
    }
  };
        ...
        <div className="w-full lg:w-[320px] shrink-0 lg:sticky lg:top-[90px] self-start space-y-4">
          ...
          {status !== 'ended' && (
            <div className="w-full bg-white dark:bg-stone-950 border border-stone-200/80 dark:border-stone-850 px-4 py-4 rounded-[6px] shadow-3xs flex flex-col items-center justify-center animate-fade-in">
              <span className={`text-[10px] font-black uppercase tracking-widest mb-3.5 flex items-center gap-1 ${
                status === 'upcoming' ? 'text-amber-500' : 'text-[#E61E25]'
              }`}>
                ⚡ {status === 'upcoming' ? 'Clearance Starts In' : 'Clearance Ends In'}
              </span>
