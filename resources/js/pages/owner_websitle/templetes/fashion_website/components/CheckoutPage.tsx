...
export const CheckoutPage: React.FC<CheckoutPageProps> = ({
    ...
    const displayCartItems = cartItems && cartItems.length > 0
        ? cartItems.map(item => ({
            ...
            image: item.selectedImage || resolveImageUrl(item.item?.display_image || item.item?.image) || 'https://images.unsplash.com/photo-1541099649105-f69ad21f3246?auto=format&fit=crop&q=80&w=400'
        }))
        : [];
                ...
                <div className="bg-white p-8 sm:p-12 rounded-sm border border-stone-200/60 shadow-2xl max-w-md w-full text-center space-y-6">
                    ...
                    <div className="pt-6 border-t border-stone-100 text-left space-y-4">
                        ...
                        <div className="relative pl-6 border-l-2 border-stone-900 space-y-5 ml-4">
                            ...
                            {/* Step 3 */}
                            <div className="relative">
                                <div className="absolute -left-[33px] top-0.5 w-4.5 h-4.5 bg-stone-100 text-stone-900 rounded-full flex items-center justify-center border-2 border-stone-900 shadow-xs">
                                    <span className="w-2 h-2 bg-[#E61E25] rounded-full animate-ping absolute" />
                                    <span className="w-2 h-2 bg-[#E61E25] rounded-full" />
                                </div>
                                <div className="text-xs">
