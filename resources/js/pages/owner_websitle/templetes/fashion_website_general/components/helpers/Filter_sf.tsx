...
export const FilterSf: React.FC<FilterSfProps> = ({
     ...
     return (
          ...
          >
               ...
               >
                    {/* Left: Active Info + Filter Button */}
                    <div className="flex items-center gap-3 shrink-0">
                         {/* Active Category Badge */}
                         <div className="hidden sm:flex items-center text-sm font-medium text-stone-850 select-none">
                         ...
                         {onOpenFilterDrawer && (
                              <button
                                   ...
                                   className="flex items-center gap-1.5 px-3 py-1 bg-white border border-stone-300 hover:border-stone-500 rounded-[4px] text-stone-800 transition-all cursor-pointer text-sm font-medium shadow-2xs hover:shadow-xs focus:outline-none"
                              >
                                   <FiFilter className="w-3.5 h-3.5" />
                                   <span>Filter</span>
                                   {hasActiveFilters && (
                                        <span className="w-1.5 h-1.5 bg-[#E61E25] rounded-full animate-pulse ml-0.5" />
                                   )}
                              </button>
                         )}
                         ...
                         {/* Search Query Tag */}
                         {searchQuery && (
                              <span className="flex items-center gap-1 text-sm text-[#E61E25] font-semibold bg-red-50 border border-red-100/60 px-2.5 py-1 rounded-[4px]">
                                   "{searchQuery}"
                                   {onClearSearch && (
                                        <FiX
                    ...
                    <div className="flex-1 flex items-center gap-1.5 min-w-0 relative px-1">
                         {/* Left Arrow */}
                         <button
                              onClick={() => scroll('left')}
                         ...
                         <div
                              ref={scrollRef}
                              className="flex items-center gap-2.5 overflow-x-auto scroll-smooth select-none flex-1"
                              style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
                         >
                              {/* All Categories Pill */}
                              <button
                                   onClick={() => onCategorySelect('all')}
                                   className={`shrink-0 flex items-center px-3 py-1 border transition-all duration-300 cursor-pointer text-sm font-medium rounded-[4px] focus:outline-none ${selectedCategoryId === 'all'
                              ...
                              {topCategories.filter((cat: any) => {
                                   ...
                                   return (
                                        <button
                                             ...
                                             className={`shrink-0 flex items-center px-3 py-1 border transition-all duration-300 cursor-pointer text-sm font-medium rounded-[4px] focus:outline-none ${isSelected
                                                  ? 'bg-white text-stone-950 border-stone-850 shadow-sm font-semibold'
                                                  : 'bg-white text-stone-600 border-stone-200 hover:border-stone-400 hover:text-stone-900 shadow-2xs'
                                                  }`}
                                        >
