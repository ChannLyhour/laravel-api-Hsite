...
export const useShippingFee = (
    ...
    const deliveryFee = useMemo(() => {
        ...
        const resolvedCoupon = nullOrRequest(appliedCoupon);
        // Check for free delivery coupon (including minimum purchase validation)
        if (resolvedCoupon?.coupon_type === 'free_delivery') {
            const minPurchase = resolvedCoupon.minimum_purchase ? parseFloat(String(resolvedCoupon.minimum_purchase)) : 0;
