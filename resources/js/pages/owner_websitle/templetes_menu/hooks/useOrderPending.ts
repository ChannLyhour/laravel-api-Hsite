...
export const useOrderPending = () => {
    ...
    const submitOrder = async (data: CheckoutData, onSuccess?: (order: any) => void) => {
        ...
        try {
            const resolvedStoreId = nullOrRequest(data.store_id);
            const resolvedCustomerId = nullOrRequest(data.customer_id);
            const normalizedData = {
                ...data,
                store_id: resolvedStoreId ? Number(resolvedStoreId) : 0,
                customer_id: resolvedCustomerId ? Number(resolvedCustomerId) : null,
            };

            const order = await ordersService.createOrder(normalizedData as any);
