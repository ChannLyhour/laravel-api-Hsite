...
export const SubcategoriesTab: React.FC<SubcategoriesTabProps> = ({ ownerId, storeId }) => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(() => {
    const saved = localStorage.getItem('itemsPerPage_subcategories');
  ...
  const handleDeleteCategory = async (id: number, name: string) => {
    const confirmed = await confirm({
      ...
      message: `Are you sure you want to delete the "${name}" sub category? This will affect linked items and any sub-categories.`,
      confirmText: 'Delete Sub Category',
      cancelText: 'Keep Sub Category',
      type: 'danger'
    });
  ...
  // Filter parents list based on levelFilter (Sub Category -> select from level 0 Root)
  ...
  const columns: HelperTableColumn[] = [
    ...
    { key: 'name', label: t('categories.sub_category_name'), align: 'left', className: 'w-[30%]' },
    { key: 'parent_name', label: t('categories.parent_category'), align: 'left', className: 'w-[20%]' },
  ...
  return (
    <div className="space-y-6 animate-fade-in font-kuntomruy">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-slate-855 tracking-tight flex items-center space-x-2">
            ...
            <span>{t('categories.sub_category_setup')}</span>
          </h2>
        </div>
      </div>
      ...
      <HelperTable<Category>
        ...
        loading={loading}
        title={t('categories.sub_category_list')}
        count={totalItems}
        ...
        bulkActions={[
          {
            ...
            onClick: async (ids) => {
              const confirmed = await confirm({
                ...
                message: `Are you sure you want to delete the ${ids.length} selected sub categories? This will affect linked items.`,
                confirmText: 'Delete',
                cancelText: 'Cancel',
                type: 'danger'
              });
              if (confirmed) {
                try {
                  ...
                  await loadCategories();
                  setSelectedIds([]);
                  toast.success('Successfully deleted selected sub categories!');
                  window.dispatchEvent(new CustomEvent('data_updated'));
                  new BroadcastChannel('data_updates').postMessage('refresh');
                } catch (err) {
                  console.error(err);
        ...
        addButton={{
          label: t('categories.add_subcategory'),
        ...
        onItemsPerPageChange={(size) => {
          ...
          localStorage.setItem('itemsPerPage_subcategories', size.toString());
          setCurrentPage(1);
        }}
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/60 animate-fade-in">
          ...
          <div className="w-full max-w-lg bg-white rounded-[5px] p-6 sm:p-8 relative z-10 border border-slate-100 shadow-2xl animate-slide-up">
            ...
            <form onSubmit={handleSaveCategory} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs sm:text-sm font-bold text-slate-700 block">
                  {t('categories.parent_category')} <span className="text-rose-500">*</span>
                </label>
                <select
                  required
