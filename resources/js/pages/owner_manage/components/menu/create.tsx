...
export const CreatePage: React.FC<CreatePageProps> = ({
  ...
  const getDraftField = <T,>(key: string, defaultValue: T): T => {
    const draft = localStorage.getItem('create_product_draft');
    if (draft) {
      try {
  ...
  const [activeTab, setActiveTab] = useState<'en' | 'km'>(() => getDraftField('activeTab', 'en'));
  const [itemNameEn, setItemNameEn] = useState(() => getDraftField('itemNameEn', ''));
  const [itemNameKh, setItemNameKh] = useState(() => getDraftField('itemNameKh', ''));
  const [selectedRootId, setSelectedRootId] = useState<number | 'new' | ''>(() => getDraftField('selectedRootId', ''));
  const [selectedSubId, setSelectedSubId] = useState<number | 'new' | ''>(() => getDraftField('selectedSubId', ''));
  const [selectedSubSubId, setSelectedSubSubId] = useState<number | 'new' | ''>(() => getDraftField('selectedSubSubId', ''));
  const [itemPrice, setItemPrice] = useState(() => getDraftField('itemPrice', ''));
  const [itemDescEn, setItemDescEn] = useState(() => getDraftField('itemDescEn', ''));
  const [itemDescKh, setItemDescKh] = useState(() => getDraftField('itemDescKh', ''));
  ...
  // Custom SKU & Barcode
  const [itemSku, setItemSku] = useState(() => getDraftField('itemSku', ''));
  ...
  const [socialMediaLinks, setSocialMediaLinks] = useState<SocialMediaLinks>(() => {
    const draft = getDraftField('socialMediaLinks', null);
    if (draft) return draft;
    const legacyDraft = getDraftField('socialMediaLink', '');
    return {
      facebook: legacyDraft || '',
      instagram: '',
      tiktok: '',
      telegram: '',
      youtube: '',
  ...
  const [itemBarcode, setItemBarcode] = useState(() => getDraftField('itemBarcode', ''));
  const [itemPurchasePrice, setItemPurchasePrice] = useState(() => getDraftField('itemPurchasePrice', '0.00'));
  const [itemStockQty, setItemStockQty] = useState<number>(() => getDraftField('itemStockQty', 100));

  const [variants, setVariants] = useState<GeneratedVariantRow[]>(() => getDraftField('variants', []));
  const [addons, setAddons] = useState<{
    ...
    const raw = getDraftField('addons', []);
  ...
  const [hasOptions, setHasOptions] = useState(() => getDraftField('hasOptions', false));

  // Extended fields
  const [productType, setProductType] = useState(() => getDraftField('productType', 'physical'));
  const [selectedBrandId, setSelectedBrandId] = useState<number | ''>(() => getDraftField('selectedBrandId', ''));
  ...
  const [shippingCost, setShippingCost] = useState(() => getDraftField('shippingCost', '0'));
  const [multiplyQtyShipping, setMultiplyQtyShipping] = useState(() => getDraftField('multiplyQtyShipping', false));
  const [brands, setBrands] = useState<Brand[]>([]);
  const [productBadges, setProductBadges] = useState<ProductBadge[]>([]);
  const [selectedBadgeId, setSelectedBadgeId] = useState<number | ''>(() => getDraftField('selectedBadgeId', ''));
  ...
  // Save Create Product draft to localStorage on state changes
  useEffect(() => {
    const draft = {
      activeTab,
      itemNameEn,
    ...
    localStorage.setItem('create_product_draft', JSON.stringify(draft));
  }, [
    activeTab,
    itemNameEn,
    itemNameKh,
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const imageFile = primaryImg?.file || undefined;
      const imageUrl = imageFile ? undefined : (primaryImg?.url || undefined);

      // Structure translation payloads
      ...
      const productSku = itemSku || `${Math.floor(100000 + Math.random() * 900000)}`;
      const variantsPayload = !hasOptions
        ...
        : variants.map((v, idx) => {
          ...
          return {
            variant_sku: v.sku || `${productSku}-VAR-${idx + 1}`,
      ...
      const finalCategoryId = selectedSubSubId || selectedSubId || selectedRootId;
      const newItem = await menuItemsService.createMenuItem({
        name: itemNameEn,
        description: itemDescEn,
        price: itemPrice,
        image_url: imageUrl,
      ...
      // Upload newly added secondary gallery images
      const secondaryNewImages = gallery.filter(img => !img.isPrimary);
      for (const newImg of secondaryNewImages) {
        try {
          await menuItemsService.addProductImage(newItem.id, {
      ...
      // Reload item to fetch all synced database entries
      const reloadedItem = await menuItemsService.getMenuItem(newItem.id).catch(() => newItem);
      ...
      <div className="flex items-center space-x-3 pb-2">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">
            Draft a brand new dish recipe and add it to your live digital menu list.
          </p>
        </div>
      </div>
                  ...
                  <div className="space-y-4 animate-fade-in">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-2.5 border rounded-[5px] text-sm focus:outline-none focus:ring-2 font-medium ${
            ...
            <div className="flex flex-col col-span-1 bg-slate-50/50 border border-slate-100 p-5 rounded-lg space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className={`w-full px-3.5 py-2.5 border rounded-[5px] text-sm font-semibold focus:outline-none focus:ring-2 transition-all cursor-pointer ${
                 ...
                 <input
                  ...
                  onChange={(e) => {
                    ...
                    setVariants(prev => {
                      const next = [...prev];
                      if (next[0]) {
                        next[0].sku = `${e.target.value}`;
                      }
                      return next;
                    });
                  }}
                  placeholder="Ex: 161183"
                  className={`w-full px-4 py-2.5 border rounded-[5px] text-sm focus:outline-none focus:ring-2 font-bold ${
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-8 pr-4 py-2.5 border rounded-[5px] text-sm focus:outline-none focus:ring-2 font-bold ${
