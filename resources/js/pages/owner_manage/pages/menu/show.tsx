...
export const ShowPage: React.FC<ShowPageProps> = ({
    ...
    const categoryName = item.category_id
        ? getCategoryPath(item.category_id, categories) || t('menu.unknown_category')
        : t('menu.unknown_category');
    ...
    const descEn = transEn?.description || item.description || t('menu.no_desc_en');
    const descKh = transKh?.description || t('menu.no_desc_kh');
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight flex items-center space-x-2">
                            ...
                            <span>{t('menu.product_details')}</span>
                        </h2>
                        ...
                        {activeTab === 'en' ? (
                            <div className="space-y-4 animate-fade-in">
                                <div className="space-y-1">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.title_en')}</span>
                                    <div className="text-base sm:text-lg font-black text-slate-900">{nameEn}</div>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.desc_en')}</span>
                            ...
                            <div className="space-y-4 animate-fade-in">
                                <div className="space-y-1">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.title_kh')}</span>
                                    <div className="text-base sm:text-lg font-black text-slate-900">{nameKh || <span className="text-slate-300 italic font-semibold">{t('menu.not_specified')}</span>}</div>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.desc_kh')}</span>
                                    <div className="text-xs sm:text-sm font-medium text-slate-600 leading-relaxed bg-slate-50/50 p-4 border border-slate-100 rounded-[5px] whitespace-pre-line font-kuntomruy">
                                        {descKh}
                                    </div>
                                </div>
                    ...
                    <div className="bg-white border border-slate-100 rounded-[5px] p-6 sm:p-8 shadow-sm space-y-6">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
                            <div className="bg-slate-50/50 border border-slate-100 rounded-[5px] p-3.5 space-y-1">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.category')}</span>
                                <span className="text-xs sm:text-sm font-extrabold text-slate-800 truncate block" title={categoryName}>{categoryName}</span>
                            </div>
                            <div className="bg-slate-50/50 border border-slate-100 rounded-[5px] p-3.5 space-y-1">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.sku')}</span>
                                <span className="text-xs sm:text-sm font-extrabold text-slate-800 truncate block">{item.sku || `PROD-${item.id}`}</span>
                            </div>
                            <div className="bg-slate-50/50 border border-slate-100 rounded-[5px] p-3.5 space-y-1">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.barcode')}</span>
                                <span className="text-xs sm:text-sm font-extrabold text-slate-800 truncate block">{item.barcode || <span className="text-slate-300 font-semibold italic">{t('menu.not_specified')}</span>}</span>
                            </div>
                            <div className="bg-slate-50/50 border border-slate-100 rounded-[5px] p-3.5 space-y-1">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.stock_qty')}</span>
                        ...
                        {attributeValuesSet.size > 0 && (
                            <div className="pt-2">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block mb-2">{t('menu.options_label')} ({attributeNames.size})</span>
                    ...
                    <div className="bg-white border border-slate-100 rounded-[5px] p-6 sm:p-8 shadow-sm space-y-5">
                        <div>
                            <h3 className="font-extrabold text-slate-800 text-sm sm:text-base flex items-center gap-2">
                                ...
                                {t('menu.variations')} ({item.variants?.length || 0})
                            </h3>
                            <p className="text-slate-400 text-2xs font-semibold mt-0.5">{t('menu.variant_breakdown')}</p>
                        </div>
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    {variantOptions.length > 0 ? (
                                                        ...
                                                        <span className="text-[10px] text-slate-400 font-bold italic">{t('menu.default')}</span>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-xs sm:text-sm">
                                                <div className="text-left">
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.purchase_price')}</span>
                                                    <span className="font-extrabold text-slate-700">${parseFloat(v.purchase_price || '0').toFixed(2)}</span>
                                                </div>
                                                <div className="text-left">
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.price')}</span>
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    {!hasVariants && (
                        <div className="bg-white border border-slate-100 rounded-[5px] p-6 shadow-sm space-y-4">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.image_gallery')}</span>
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{t('menu.status')}</span>
                                {item.status === 'active' ? (
                    ...
                    <div className="bg-white border border-slate-100 rounded-[5px] p-6 shadow-sm space-y-4">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">{t('menu.general_info')}</span>
                            {hasVariants && (
                                item.status === 'active' ? (
