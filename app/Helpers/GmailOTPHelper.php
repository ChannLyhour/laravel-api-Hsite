...
class GmailOTPHelper
{
    ...
    public static function sendOTP ($order, $otpCode)
    {
        ...
        try {
            ...
            $isStoreSendmail = (strtolower(trim((string) $mailMailer)) === 'sendmail') || env('MAIL_MAILER') === 'sendmail';
            $isStoreSmtpConfigured = !$isStoreSendmail && $mailHost && $mailUsername && !empty($cleanMailPassword);
            $isStoreGmailConfigured = !$isStoreSendmail && ($gmailEnabled === '1' || $gmailEnabled === 1 || $gmailEnabled === 'true') && $gmailEmail && !empty($cleanGmailPassword);
