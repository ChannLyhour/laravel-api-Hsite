...
class GmailOTPHelper
{
    ...
    public static function sendOTP ($order, $otpCode)
    {
        ...
        try {
            ...
            $isStoreSendmail = !$isResendApi && ((strtolower(trim((string) $mailMailer)) === 'sendmail') || env('MAIL_MAILER') === 'sendmail');
            $isStoreSmtpConfigured = !$isResendApi && !$isStoreSendmail && $mailHost && $mailUsername && !empty($cleanMailPassword);
            $isStoreGmailConfigured = !$isResendApi && !$isStoreSendmail && ($gmailEnabled === '1' || $gmailEnabled === 1 || $gmailEnabled === 'true') && $gmailEmail && !empty($cleanGmailPassword);
            ...
            if ($isResendApi) {
                ...
                $finalFromEmail = ($mailFromAddress && !$isUnverifiedDomain) ? $mailFromAddress : 'noreply@vhsitekh.site';
